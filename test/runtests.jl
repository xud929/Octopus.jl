...
@testset "CUDA GaussianPIC coupled subtraction matches CPU" begin
    ...
    # indexed-wavefront route. Parity is checked with green_cache=:none for the
    # same reason as the existing GaussianPIC parity test: the slice-pair cache
    # makes a discrete reuse/rebuild decision from moments that differ between
    # backends at 1e-16, which can flip the decision and change the grid.
