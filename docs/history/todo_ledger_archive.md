# TODO ledger archive (frozen 2026-08-16)

This is the complete TODO file as it stood on 2026-08-16, frozen verbatim when
the live ledger was restructured into three documents:

- [`../todo.md`](../todo.md) now carries ONLY unfinished work;
- [`../experiences.md`](../experiences.md) carries the recurring lessons this
  record taught, distilled and grounded in the incidents below;
- everything else — closed rows, campaign narratives, measurements, wrong
  turns and the corrections kept beside them — lives HERE, unchanged, with
  every U-number and row text intact, so code and test comments citing a
  "todo row" resolve against this file.

The handful of rows still open at the freeze exist twice: live copies in
`../todo.md` (which will evolve), frozen copies here (which will not). Nothing
in this file should be edited; it is history, and its wrong claims are kept
readable next to their corrections on purpose. (One mechanical exception was
applied at the freeze: relative links were retargeted for this file's new
location under `history/` — path adjustment only, no content change.)
...
>
> **Nothing actionable is queued: every remaining open row is blocked or
> heavy.** U4-1 closed 2026-08-16 (survivable corner probe + NaN-aware
> comparator; the aperture half of the effectiveness contract can finally
> fail, and its can-fail control is in the suite). U9-2 closed 2026-08-15
> (all 23 body keys on all six lattice-magnet kinds, 83 new entries
> checked). The remaining rows and what they wait on: U4-18/U12-8
> (constructor-keyword introspection), U12-6 (semantic checking), and the
> Bmad reference cases (five riders, one Bmad-in-the-validation-path
> setup closes all of them). Watch-only: the CI segfault flake tally.
>
...
> [`history/neighbour_audit_2026_08_14.md`](neighbour_audit_2026_08_14.md);
> the remaining open rows are blocked-or-heavy
> (U4-18/U12-8 on constructor-keyword introspection, U12-6 on semantic
> checking, and the Bmad reference cases — now carrying five riders — on
> a tool).
>
> | item | state | notes |
> |---|---|---|
...
> | ~~RF cavity velocity-slip term~~ | **closed (2026-08-14)** | `_rf_kick` converts with arc position `s = 0` because a runtime element has no channel to its accumulated reference path, so a ring closed by `ThinRFCavitySpec` has slip factor `alpha_c`, missing `-1/gamma0^2` — measured 1.84x nu_s error at 2.5 GeV proton / alpha_c = 0.2, wrong transition side when `alpha_c < 1/gamma0^2`; negligible at EIC-class gamma0, which is why every committed check passed. The theory note's own §8 item 4 criterion (nu_s against the **full** eta) is the missing check (this row said §7 until the 2026-08-05_b audit; the same wrong reference was hand-copied here from the note). Fix needs the arc/survey channel — the same one Scope B needs for `P0(s)`; fold them. Model boundary now documented on the element and in the note (correction block, §6 Step 0). **Design decided (2026-08-13):** [`design/survey_and_reference_channel.md`](../design/survey_and_reference_channel.md) — static survey baked at compile via the line walk, dynamic part from `TrackingContext.turn`, loud refusal without context; rejected alternatives and the ramping/multipass taxonomy recorded there. **Closed (2026-08-14):** the geometric survey channel landed — one arc walker (`_collect_spec_s!`) shared with the aperture arc positions, a `_bind_apertures`-style rebind with a count tripwire (a cavity hidden in a kept-whole sub-line refuses loudly), and the survey benchmarked *externally* against MAD-X `SURVEY` element-for-element (`MADXSurveyConsistencyContract`, 5 fixture lines incl. nesting, `reverse` vs `-half` reflection, 0.7 rad bends; worst deviation 1.07e-14). The cavity applies the exact velocity slip as a symplectic z-shift `z += Δs·(β/β₀−1)` per kick (Δs = arc from the previous kick, wrapping the turn; `_velocity_slip_g` cancellation-free, pinned vs BigFloat at rtol 1e-12 up to γ₀ ≈ 2e4). Measured at this row's own working point: ν_s lands on the **full-η** prediction (in-suite FFT tune, rtol 5e-3), the 1.84× ratio is pinned as a corrected-vs-bare A/B, and the wrong-transition-side flip (`α_c = 0.10 < 1/γ₀²`: corrected unstable at phase 0, uncorrected happily oscillating) is demonstrated. Two plausible-but-wrong forms — constant `s` in the conversion (cancels; measured ν_s stayed α_c-only) and accumulated `turn·C` (unbounded state) — are kept as measured negative results in [`theory/arc_survey_and_velocity_slip.md`](../theory/arc_survey_and_velocity_slip.md) §4; the design note's §3c carries the matching correction block. The `TrackingContext.turn` half of the design was NOT needed. Scope B now lacks only the energy half of the channel. |
...
> | Curved-girder misalignment survey via the floor plan | **open (2026-08-14_b audit, opportunity)** | The kept-whole line's misalignment wrap builds both faces from a STRAIGHT 1D survey (`compile_runtime(:line)`, U15-2): for a misaligned line whose contents bend, the exit patch is wrong at first order in the bend angle — measured `dx·theta`, recorded in `beam_line_composition.md` §10 and accepted when no geometry layer existed. `_floor_step`/`survey` now exist and compose the exact per-element frames the wrap needs; routing the line wrap's survey through them would close the limitation. Not a defect (the recorded behavior stands, loudly documented); an upgrade priced at: rework `compile_runtime(:line)`'s geom construction + a curved-girder testset against the same rigid-displacement oracle U15-2 measured with. |
> | ~~Crab cavity velocity slip~~ | **closed as assessed-negligible (2026-08-14)** | `ThinCrabCavity` reads its phase as `k·z` (`src/elements/crab_cavity.jl`) — the same arrival-time-read-from-a-path-coordinate class the F16 fix just closed for `ThinRFCavitySpec`, unassessed for the crab. Two differences from F16 keep this a lead rather than a defect: the crab's phase already differs from the RF's `k·z₁` by the recorded `β` factor (its docstring quantifies 4.6e-3 rad at 2.5 GeV/7 mm), and a crab kick is transverse — the slip mistunes the *crabbing phase* rather than the synchrotron frequency, so the observable is luminosity/crossing geometry, not ν_s. Assess whether `ds_turn·(β/β₀−1)` matters at production crab parameters; the survey channel and `_velocity_slip_g` are already in place if it does. Found by the F16 neighbour walk (re-read sibling elements after a fix); do not fix in passing. **Assessed (2026-08-14), measured on the crab's own scale (`k·σ_z`):** after F16 the residual is the *within-turn* slip only (bounded, `Δs < C`, reset each turn at the ring cavity) — EIC 275 GeV proton: 1.8e-4 rad of crabbing phase (7.4e-4 relative), far below RF phase-control tolerance; 10 GeV electron: 8e-8 rad; the regime where it would matter (2.5 GeV, C = 100 m: 0.118 rad) is F16's own and hosts no crab. Boundary now quantified in the `ThinCrabCavity` docstring beside the recorded β-factor boundary; not wired, machinery stands ready. |
...
> | ~~Legacy moment observers host-copy the whole beam per turn~~ | **closed by removal (2026-08-11, owner decision)** | Rather than build a second device moment pipeline for observers nothing in the tree used, `BeamMomentObserver` and `JLD2BeamMomentObserver` were REMOVED in the pre-release window: structs, writers, schemas, exports, metadata probes, and their suite pins (U7-2 atomic-update, U7-3 quadratic-size, U7-8 layout tripwire — their lessons live in the dated audit records). The JLD2 READER branch stays so archived `.jld2` files remain readable (`read_moment` documents the removal); the binary `.bin` format never had an in-tree reader — its layout is recoverable from git history (`_initialize_moment_file!` before `3a65c7e`: `Int32` row count, `Int32` format-string length, CSV column names, then `Float64` rows). The U7-10 second-writer registry test re-vehicled onto `MomentObserver`. [`neighbour_audit_2026_08_11.md`](neighbour_audit_2026_08_11.md). |
> | ~~`BPMObserver` path mode opens its file every observed turn~~ | **closed (2026-08-15)** | Same open/append/close-per-turn shape the `.lum` observer had (2.3 ms/turn on the production cluster FS), multiplied by however many BPMs a study reads. Capacity buffering is the worked recipe (`LuminosityObserver`, `3cc0158`), but BPM replay rewrites the file from object memory, so the buffer must compose with that replay model rather than the `.lum` row-trim. **Closed:** the BPM's full-memory model made the composition SIMPLER than the `.lum` row-trim — the file is always rebuildable, so buffering is one `flushed` cursor: readings `[1:flushed]` are on disk, the threshold flush appends the tail in one open, `finalize_observer!` (both chains, success and failure paths) flushes at every `execute!` end, and the replay rewrite resets the cursor to the end so the two mechanisms cannot disagree. `capacity = 1` default preserves the per-reading-durable behavior exactly (the `.lum` precedent's choice); schema/report metadata landed with it. Suite-pinned: block flushes, finalize on both chains, replay-leaves-each-turn-once at capacity 100, capacity guard. |
...
> | Lattice-magnet schemas under-declare what `_lattice_magnet` reads | **closed (2026-08-15, U9-2)** | `_lattice_magnet` reads the same 23 body keys for every kind it compiles, while each kind's `parameters` declaration covers only its own subset: drift is missing 21 of them, quadrupole/sextupole/octupole/multipole 15 each, sbend 2. The WARNING half is closed — `_extra_tracked_keys` stops the unknown-key warning telling a user a key is "NOT being tracked" when the runtime reads it (measured: `e1` on a quadrupole shifts tracking by 7.7e-7, up to 5.8e-2 for others), verified with a negative control that a genuine typo still warns. What remains is completing the schemas themselves, which is the real repair: it also makes `element_help` accurate and puts these keys under `ElementParameterEffectivenessContract`. The reason it is not done here is blast radius — it adds ~80 parameter entries to the sweep, and each must then be verified as effective or exempted with a reason, on kinds where many are structurally inapplicable (a drift has no `e1` face angle to act on). Splicing a shared `_LATTICE_BODY_PARAMS` NamedTuple into the six kinds, the way `_PLACEMENT_PARAMS` is spliced, is the shape; `_LATTICE_BODY_KEYS` in lattice_magnets.jl is already the authoritative list to derive it from. *(Closed 2026-08-15, in exactly that shape but with the derivation REVERSED: `_LATTICE_BODY_PARAMS` — 23 metas, one table — is merged into all six schemas via `parameters = merge(_LATTICE_BODY_PARAMS, (...))` with curated meanings winning, and `_LATTICE_BODY_KEYS` is now `keys(_LATTICE_BODY_PARAMS)`, so the table is the single source. The `_extra_tracked_keys` side list is gone: the warning consults schemas that now tell the truth. The priced blast radius resolved better than priced — the sweep took all 83 new entries as CHECKED, 414 → 499 with unperturbable/rejected unchanged at 42/23 and ZERO new exemptions, at the `_LATTICE_BODY_PROBE` working point: the FINT/HGAP correction needs the dipole field and both factors nonzero; `curved_order` must be 3, not the default 8, for its step to move anything; and the multipole needed its own point (curved_order=6 with named normal content at every order) because a normal `K_n` needs `curved_order ≥ n+1` to reach the curved body at all while the truncation STEP is invisible without high-order content to correct — measured floor `k5s` at 8.7e-19, one bit, which `atol=0` counts. Two exemptions RETIRED rather than added: a drift probed with kick content genuinely splits, so `drift.nst` and `drift.integrator_order` moved from documented-inactive to checked, and the can-fail control's exemplar moved to `marker.x_offset`, whose inertness is map structure no probe can undo. `element_help` now tells the truth on all six kinds; the shared `_LATTICE_BODY_HELP` sentence names every body key, so the metadata validator's mention check holds without six hand-copied enumerations. Pinned by the "Lattice-magnet body schemas are complete (U9-2)" testset: per-kind key coverage, curated-meaning survival through the merge, `e1`-on-a-quadrupole and `b0`-on-a-drift moving the map, `va` on the sbend, and the typo negative control.)* |
> | ~~`misalign_convention` is inert at every element probe~~ | **closed (2026-08-15, the alternatives keystone)** | The MAD-X-vs-Bmad rotation composition order — the thing the PTC contract's `quad_mis_all` / `cfbend_mis_all` cases exist to pin — cannot be observed at any shipped probe on any of the 25 kinds. Each probe is in one of two states and neither shows it: no placement offset at all (the misalignment wrap is never built, so the convention is inapplicable), or exactly ONE offset (`x_offset=1.0e-3` and nothing else), where measured `max|madx - bmad| = 0.0` — the documented fact that one rotation at a time cannot distinguish the two composition orders. With all six placement degrees of freedom nonzero, the `quad_mis_all` shape, the conventions differ by 1.294e-4, so the parameter is perfectly observable and the probes simply never reach where it acts. The REPORTING half is closed: U4-2's accounting now counts it in `:unperturbable` instead of dropping it silently, so the contract no longer claims it was checked. What remains is (a) probe overrides giving the misalignment-carrying kinds all six placement DOF, and (b) a way to perturb a Symbol parameter at all — `_perturb_candidates` returns `()` for every Symbol because no declared alternative-value metadata exists to derive from. (b) is the general blocker and would also unlock `fringe`, `bend_model`, `aperture.shape`, `patch.convention` and `xy_coupling.mode`; adding an `alternatives` field to `ParamMeta` is the obvious shape, and would let the sweep derive rather than hand-list. **Closed exactly by that shape (2026-08-15):** `ParamMeta` gained `alternatives` (the machine-readable enum; declared alternatives replace the generic candidate rule, so they serve Reals with hand-picked moves too), `_perturb_candidates` derives from it, and every effectiveness probe now carries all six placement DOF through ONE central merge (`_PLACEMENT_PROBE_DOF`) — example-derived fallback probes enriched identically to curated ones. Declared and now CHECKED: `misalign_convention` on every kind, `fringe` (with the quadrupole probe given `kn=(0,1.7,5)` so `highest_fringe` has content to cap at 2), `bend_model`, `patch.convention`, `xy_coupling.mode`. Contract: 382 → 414 checked, 74 → 42 unperturbable, zero new exemptions — every kind's map showed the convention once asked with all six DOF. Suite-pinned: the candidate derivation, current-value exclusion, Real-alternatives replacement, and the two conventions producing different quadrupole maps at the full-DOF probe. |
> | The aperture half of `ElementParameterEffectivenessContract` cannot fail | **closed (2026-08-16, U4-1)** | The `:aperture` probe sets limits so tight the probe particle is killed, so the baseline is six `NaN`s. The decision is `maximum(abs, moved .- baseline) <= atol`, and `NaN <= 0.0` is `false`, so the pair is never pushed to `ignored` and is scored effective — for all 11 aperture parameters (`x_limit`, `y_limit`, `dx`, `dy` and the seven placement params), whatever the map does. Verified directly: baseline and perturbed are both all-`NaN`, the difference is `NaN`, and the comparison returns "changed". Two things must land together, which is why this is not a one-liner. (1) The comparison must be NaN-aware — `isequal` elementwise, so both-`NaN` reads as UNCHANGED and `NaN`-vs-finite reads as changed. (2) With that alone the 11 parameters flip from silently passing to failing, because the probe cannot distinguish them: a killed particle produces no observable difference for any perturbation. A single probe cannot fix it either — outside in x only makes `x_limit` falsifiable and leaves `y_limit` dead; outside in both kills every parameter's signal. It needs per-parameter probes, or those pairs declared in `inactive` with reasons (the contract's own documented mechanism). Constructive direction found while fixing U25-4 (2026-08-06): an aperture's map is the IDENTITY for every particle that survives it, so the only observable a perturbation can produce is an alive/dead flip — no tolerance on coordinates will ever help. And `_perturb_param` only ever *increases* a `Real` (`+0.13`), so a limit can only become more generous, which from a killed baseline is the one direction that does show a flip and from a surviving baseline shows nothing. A falsifiable design therefore needs a probe where the particle SURVIVES plus a per-kind shrinking perturbation for the limits, or per-parameter probes. Note that `_contract_coordinate_metrics` was made NaN-aware for U25-4 and is a worked example of the alive/dead accounting (`lost_both_backends` / `lost_one_backend` / `compared_coordinates`), but it is a DIFFERENT comparator — this contract compares raw coordinate tuples and was not touched. *(2026-08-15: the `alternatives` mechanism now exists and serves Real parameters with hand-picked values — the shrinking-limit moves this row asks for are one declaration away once the probe survives; the survivable probe redesign is what remains.)* Note the interaction with the `checked >= 353` floor raised for U17b-3: 11 of those 353 are currently unfalsifiable, so the floor is honest about the count and not about the coverage. *(Closed 2026-08-16, both halves landing together as the row demanded. (1) `_probe_deviation`: both-NaN reads UNCHANGED via `isequal`, NaN-versus-finite reads as a definite change — the alive/dead flip — and a fully dead baseline now reads IGNORED loudly; the killing probe re-run as the suite's can-fail control fails naming `aperture.x_limit`. (2) The survivable CORNER probe: the particle sits inside the rectangle but outside the inscribed ellipse (frame position ~(+1.29e-3, −1.02e-3) against limits 1.45e-3/1.15e-3 after the merged placement DOF), so `shape`'s declared alternatives (:ellipse, :rectellipse) both kill, the limits' declared shrunken alternatives (1.0e-3, 8.0e-4) kill — the alternatives mechanism serving Real parameters exactly as anticipated — and dx/dy/x_offset/y_offset kill at the generic +0.13, with tilt/ref_tilt killing too because the margins (1.6e-4/1.3e-4) sit inside the ~2e-4 arc a 0.13-rad rotation sweeps at that radius. z_offset/x_pitch/y_pitch/misalign_convention cannot flip at any generic candidate (boundary-relative shifts O(1e-5)); they are checked through the placement wrap's measured last-bit deviations (1e-19..1e-17), genuine consumption evidence, with zero new exemptions. Contract movement: 499 → 500 checked (aperture.shape newly perturbable), 42 → 41 unperturbable — the count moved by one, the COVERAGE by twelve: the 11 NaN-fake checks became real, 9 by flip.)* |
...
[`docs/history/slice_longitudinal_interpolation_record.md`](slice_longitudinal_interpolation_record.md);
...
See [`docs/history/poisson_solver_review_2026_07_24.md`](poisson_solver_review_2026_07_24.md)
...
3. ~~**CPU indexed-slice path.**~~ **CLOSED NEGATIVE (2026-07-25).** Implemented
   ...
   Section 19 of
   [`poisson_solver_review_2026_07_24.md`](poisson_solver_review_2026_07_24.md).
...
[`docs/history/gaussian_slicing_convergence_2026_07_31.md`](gaussian_slicing_convergence_2026_07_31.md).
...
[`docs/history/ref_tilt_2026_08_02.md`](ref_tilt_2026_08_02.md).
