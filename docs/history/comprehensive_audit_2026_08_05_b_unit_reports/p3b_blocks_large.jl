...
println("### N = $N  threads pinned to 256, requested BLOCKS varies (explicit > 256 is allowed)")
ref = nothing
for blocks in (:auto, 256, 512, 1024, 2048)
    ...
    global ref
    ref === nothing && (ref = r.g)
    @printf("  blocks=%-5s eff=%4d | varx ulps=%d covxpx ulps=%d mpy ulps=%d sx ulps=%d  varx=%.17e\n",
        string(blocks), r.L.blocks,
        ulps(r.g.moments.a0, ref.moments.a0), ulps(r.g.covxpx, ref.covxpx),
        ulps(r.g.mpy, ref.mpy), ulps(r.g.sx, ref.sx), r.g.moments.a0)
end
