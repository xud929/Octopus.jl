...
- **U13-2 completion** (`980bb51`): placement parameters declared in every
  kind's schema (shared `_PLACEMENT_PARAMS`; the 13 kinds with richer own
  entries keep them and gain `ref_tilt`); effectiveness contract now 353
  pairs, 0 ignored/broken/skipped. Inactive entries derived from map
  structure, not the bitwise sweep — the (v-d)+d frame round trip makes a
  mathematically inert conjugation move the last bit (measured on the
  reverse Lorentz boost and the marker rotations). Full-coefficient probes
  for the three linear maps; anisotropic damping probe for
  lumped_radiation.
- **REGRESSION found and fixed** (`05c8ed5`): the hygiene sweep's U15-4
  throw interpolated the RNG method code into its message — a heap string,
  invalid device IR — so every tracking line containing a counter-RNG
  element failed `cuda_track_kernel!` compilation outright, original
  committed backend-consistency line included. The suite never tracked a
  stochastic element on the fused CUDA path, so the campaign gate stayed
  green; caught by the U21-5 line extension's first GPU run. Message now
  static; CUDA-gated suite pin (LumpedRad-only contract) holds the class.
- **U21-5** (`3fd4826`): backend-consistency line covers all 30 registered
  kinds (was 11 of the 29 declaring), with a declaration-coverage tripwire
  in the script; both legs green, CPU/CPU 0.0, CPU/GPU 1.7e-16. **U21-6**
  (same commit): the PTC generator's dead `Case.octopus` spec copy deleted
  (it had drifted: a strengths-less multipole, an unregistered `:rbend`);
  reference TSV regenerated byte-identical under MAD-X 5.03.06.
- **U21-4** (`004de64`): the symplecticity script consumes
  `_symplecticity_contract_cases()` — one case list, two independent
  evaluators; it had checked 8 of the contract's 12 cases while both sides
  claimed mirror status. All 12 pass the script's own FD evaluator.
- **U4 observations** (`52a99ab`): `_collision_solver` equality is now
  type + `solver_configuration` (identity refused independently built
  equal solvers); mixed-IP schedule turns drop their .lum row loudly with
  the remedy named (writing NaN would corrupt the evaluated-and-failed
  meaning). **U2-3** (`714c87e`): CUDA TSC w3 in the CPU closed form; 528
  points bit-identical, bases and weights. Both pinned (`a32aab6`).
- **U19-4/9/10 + U20 framing** (`3151b48`): the kernel-sign selection is
  gated (selected sign must satisfy the translation-mode criterion:
  measured 1.11e-5 vs 1.16 for the rejected sign); gate-vs-characterize
  framing added to the Vlasov header and the five unframed README sections;
  the two Gaussian-PIC field studies labeled as validating a local
  reimplementation, with the z-scan named as the production-internals
  study. The 21 per-unit audit reports — this queue's file:line ground
  truth, until now session-scratch and nearly lost — are archived under
  `comprehensive_audit_2026_08_05_unit_reports/` and indexed.
- **U16-6, U17-6/7/8** (`04dff70`): last silent CUDA gate skips visibly;
  deterministic workspace-reentrancy variant (hold a lease across a full
  collide) replaces scheduler luck; the abort-ordering caveat documented in
  the header; the dropped-charge warn pin checks message and count, and the
  equal-extent negative control now proves the equality does work.
- **U21-8/9/11/12, U18-3/5** (`af4f53e`): z-scan's fourth output
  documented and its dead helper removed; the option-consistency summary
  groups by beam with per-beam sigmas and announces baseline-less runs;
  the slicing-convergence "correctness check" says it is print-only and
  drops its dead config field; the paper's luminosity-anchor TSV is marked
  hand-transcribed; the suite cleans up the example artifacts it creates;
  a relative turn-timing path resolves beside the tests as the header
  claims.
- **U13-5 full** (`da3a8cf`): 336 of 336 exports documented, suite
  tripwire keeps it so. Structural finding: a comment line between a
  docstring and its definition silently DETACHES the docstring on Julia
  1.12.4 — ten exports had complete docstrings attached to nothing via the
  house `T<:Number` comment convention; a const alias of a parametric
  struct likewise does not inherit docs.

- **Follow-up the full-suite gate caught** (`bbd48df`): the U4
  `_collision_solver` fix's blast radius included
  `PublicConfigurationEffectivenessContract`, whose solver-mismatch probe
  built two IDENTICAL solver objects and expected rejection — a probe of
  the retired identity rule. The targeted tests around the fix missed it;
  the final full suite did not. The probe now differs in substance
  (min_sigma), and a complement probe pins acceptance of equal
  configurations in distinct objects. Worth keeping: when a fix changes
  REJECTION semantics, grep for the contracts that probe the old rejection
  before calling the fix done.

With these, every item in the priced still-open list above is closed: the
queue from this audit is EMPTY. What remains open repo-wide is tracked in
`docs/todo.md` (RF velocity-slip + arc channel, Bmad reference cases, and
the validator narrowings recorded there).
