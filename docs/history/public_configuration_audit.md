...
| Gaussian solver | kick overrides, luminosity scale, slicing, `min_sigma`, luminosity sampling beam, centroid controls, longitudinal kick, virtual drift, coupled covariance, CUDA batch mode | Structured metadata covers all physics and scheduling choices; inherited slicing is retained separately from requested overrides. Invalid numerical, drift, and scheduling choices are rejected. |
