...
> | **RF cavity** | **designed, ready to implement** | No cavity exists, so the longitudinal plane is open and Twiss is blocked. Design: [`docs/theory/rf_cavity_and_reference_energy.md`](theory/rf_cavity_and_reference_energy.md). Scope A — constant reference energy, drift-kick-drift, `e0` as an element parameter — unblocks Twiss and needs no reference machinery. Scope B (accelerating) is a *separate element kind* following Bmad, not a flag, because the phase convention differs between them. **Start here.** |
