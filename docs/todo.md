...
> | Solver-option effectiveness contract | **open, highest value (2026-08-03)** | `ElementParameterEffectivenessContract` sweeps all 238 *element* parameters mechanically; nothing does the same for *solver* options, and `PublicConfigurationEffectivenessContract` checks the CUDA PIC launch families for `PICPoissonSolver` only. That gap is exactly where the [part 2 audit](history/comprehensive_audit_2026_08_03_part2.md) found S1 and S2: `GaussianPICPoissonSolver` discarded every launch override and the policy thread count (measured: 0 `:cuda_pic_launch` receipts against PIC's 51) while `configuration_report` reported them `resolved`, and a CUDA-only `batch_mode` on the soft-Gaussian solver was inactive on CPU with no warning anywhere. Both are fixed; the *class* is not closed. Copy the element contract's shape — build each solver through its constructor, perturb one declared option, assert the change is observable at its declared `consumer` (execution receipts for execution options, coordinates for numerical ones). |
> | Audit the strong-strong solver stack | **open, narrowed (2026-08-03)** | The [part 2 audit](history/comprehensive_audit_2026_08_03_part2.md) read `interface.jl` (2,093) in full, so the *public configuration surface* is now covered. ~11,300 lines of solver kernels remain: `pic_cuda.jl` (5,807), `pic_cpu.jl` (1,715), `gaussian_pic_cuda.jl` (1,154), `spectral.jl` (1,045), `spectral_cuda.jl` (760). **Read `pic_cpu.jl` first** — it is the CPU reference every parity contract validates the CUDA paths against, so if it is wrong the contracts agree on the wrong answer. Last reviewed 2026-07-24 and 2026-07-26. The defects found so far were each a *cross-product* — two features that work alone, combined in a way no doc section covers — which is the shape to look for. |
