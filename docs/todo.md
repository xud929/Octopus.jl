...
> | ~~Beam line / sequence~~ | **implemented (2026-08-02)**, one piece left | `BeamLine` in `src/elements/beam_line.jl`, per the design in [`docs/theory/beam_line_composition.md`](theory/beam_line_composition.md). Tracks identically to the equivalent bare tuple; girder rigidity measured at 0.0. **Outstanding:** no `@element_spec` metadata block, so `:line` has no `element_help` entry and does not appear in `summarize_registry()` — registering it puts `entries` in front of `ElementParameterEffectivenessContract`, which perturbs every declared parameter and needs a considered answer rather than a quick one. **Start here.** |
