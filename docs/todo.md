...
> | Audit the strong-strong solver stack | **open, narrowed again (2026-08-03)** | `interface.jl` (2,093) was read in full in [part 2](history/comprehensive_audit_2026_08_03_part2.md) and **`pic_cpu.jl` (1,715) in [part 3](history/comprehensive_audit_2026_08_03_part3.md)**. ~9,600 lines remain: `pic_cuda.jl` (5,807), `gaussian_pic_cuda.jl` (1,154), `spectral.jl` (1,045), `spectral_cuda.jl` (760), `gaussian_pic.jl` bulk. **The "contracts agree on the wrong answer" worry was correct, and part 3 caught it happening**: the default Green cache expands the mesh with the node count fixed, which changes the cell size and breaks the integer-cell alignment `green_type = :lattice` requires — both backends broke it identically, so CPU/CUDA parity passed at 1e-13 on the field of a source displaced 0.4 cells, and the only analytic field validation bypasses the cache. **Take `pic_cuda.jl` next with a specific question rather than a general one**: its cached-prep path mirrored the CPU one closely enough to inherit that defect verbatim, so look for the other invariants it mirrors. The cross-product shape still holds — part 3's third defect is `grid_extent` bit-identically erased by `interaction_grid`. |
> | Reconcile `:lattice`'s measured field accuracy with the theory note | **open (2026-08-03)** | [Part 3 §9](history/comprehensive_audit_2026_08_03_part3.md) measured `:lattice` at a median relative field error of 3.0e-2 against `:integrated`'s 1.4e-3 on the 11:1 beams at grid 128 — `:lattice` **20x worse**, where [`pic_free_space_kernels.md`](theory/pic_free_space_kernels.md) §3.4 records it as 1.48x *better* (1.348e-2 vs 1.991e-2). The setups are not comparable (part 3 used a 40,000-point noiseless quantile source sampled within ±3σ; the note does not state its field-point set), so **this is not a claimed contradiction** — but the ordering is inverted and `:lattice` is recommended precisely for flat-beam field-accuracy work. Needs the note's exact measurement reproduced. Cheap: the harness is `validation/pic_gaussian_field_validation.jl`. |
