...
> | ~~Concurrency-test the CUDA paths~~ / the rest of the solver stack | **CUDA half closed (2026-08-03)** | The [part 4 audit](history/comprehensive_audit_2026_08_03_part4.md) §3.5 swept `pic_cuda.jl` for the closure-capture class on lowered code: **288 methods, 0 carrying a `Core.Box`**. Also checked the two other concurrency surfaces this file has — the four concurrent field streams (per-stream charge buffers, freshly allocated FFT scratch, no shared mutable state) and `_cuda_pic_add_time!`'s non-atomic accumulate (safe: no yield point between the read and the write, and the tasks are `@async`, not `@spawn`). What remains open below is the *non-CUDA* half of this entry. |
> | Concurrency-test the rest of the solver stack | **open (2026-08-03)** | The [2026-08-03 audit](history/comprehensive_audit_2026_08_03.md) found **three** threading defects, and every one was invisible to the test suite because `Pkg.test()` ran single-threaded and `_run_logical_workers` has a `nworkers == 1` fast path that never spawns. CI now runs `--threads=4`, which is what exposed the `:equal_area` slicing corruption. Two of the three were the *same* Julia trap: a name assigned both in a `do` block and in its enclosing function is one shared `Core.Box`, because `if` does not open a scope in Julia and `for` does. **Sweep by searching lowered code for `Core.Box`, not by grep** — a text sweep gave six false positives and missed a real one. All 15 `_run_logical_workers` sites are now clean; the CUDA kernels have never been checked for anything equivalent. |
> | Sweep the `h != 0` x transverse-field cross-product | **open (2026-08-03)** | The [2026-08-03 audit](history/comprehensive_audit_2026_08_03.md) found the same defect twice, in two files: a straight multipole kick in a curved frame is a gradient **iff `Im f == 0`** for the analytic `f(w) = sum (K_n + i Ks_n) w^n/n!`, i.e. iff the content is a pure NORMAL dipole. `lattice_magnets.jl` exempted the whole dipole order and let the skew dipole through (2.5e-3); `solenoid.jl` had no routing at all, so every order was affected (9.6e-4 with `k1`, 3.2e-2 with `k0s`, and **`nst` does not remove it**). Both fixed. What is open is the general sweep: any element accepting a curvature *and* a transverse field should be checked against that condition rather than assumed. |
> | ~~Solver-option effectiveness contract~~ | **done (2026-08-03)** | `SolverOptionEffectivenessContract` in `src/contracts/Contracts.jl`: 68 options checked on CPU, 2 CUDA launch surfaces, 13 CUDA-only deferred, 7 exempted with reasons. An option with neither a declared alternative nor a stated reason fails, so the tables cannot fall behind the schema. Discriminating power verified by reintroducing the GaussianPIC launch defect — it fails naming that solver, and passes when restored. Building it found six more defects, recorded in [part 2 §15](history/comprehensive_audit_2026_08_03_part2.md): an `UndefVarError` in the **default** integrated Green kernel on any on-axis node, a `:solver_runtime` receipt that covered 29 of 88 declared options, a mesh-extent estimator the Gaussian-PIC hybrid silently dropped, two method ambiguities (one a public `loss_summary` `MethodError`), and two options declared execution-only that measurably change results. **Two lessons worth keeping**: the first version decided what to test by calling the very function the defect had broken, so it skipped the solver instead of failing it; and its result depended on the caller's RNG state, which is what then exposed the Green-kernel crash. |
> | **Verify and fix the eight recorded findings from part 6** | **open (2026-08-03)** | [Part 6 §5](history/comprehensive_audit_2026_08_03_part6.md) records eight agent-confirmed findings that were deliberately **not** independently verified and **not** fixed, each with a reproduction. Headed by **R1**: CUDA `:equal_count` slicing is not equal-count when z has ties — CPU counts `[222,222,222,222,223,...]` vs CUDA `[211,196,236,238,163,...]`, a **27% relative error in a slice weight**, and slice weights multiply `kbb` directly. Then **R6/R5** (`MomentObserver` crashes or silently truncates in ordinary chunked runs — a documented workflow), **R3** (the Symbolics adapter can never activate: the package is in no section of `Project.toml`, so the error message tells users to install what they already have), **R4** (`@knob p::T` changes a value with no epoch bump, leaving dependents and compiled runtimes stale). **Verify before fixing** — part 6 §6 records this session's agent hit rate at ~60%. |
> | ~~`spectral.jl` (1,045) — completely unaudited~~ | **open (2026-08-03)** | Its agent was stopped mid-read during part 6, so the file is exactly as unaudited as before. It is also the CPU reference `spectral_cuda.jl` was compared against, so auditing it firms up several of part 6's own conclusions. |
> | **Part 7 queue — 26 agent-confirmed findings, NONE verified by me** | **open (2026-08-04)** | [Part 7](history/comprehensive_audit_2026_08_03_part7.md) records the last ~4,100 lines. **Take T3 first: an unchecked `@inbounds`/`CUDA.@atomic` write past the end of `counts`**, reachable from an ordinary nested-vector line, reported to corrupt a heap word *and* make a collimator's kills show as `unattributed`. Then **T1** — the knob epoch never fires for any task built from a `BeamLine`, the consumer-side twin of the producer-side defect fixed in part 6, so `set_knob!` changes nothing while `knob_value` reports the new number. T1/T3/T4/T5 share one root cause (`LineEntry` is not an `ElementSpec`, and five walkers handle that inconsistently), so fixing it may close four at once — check that first. Then **K1** (`RBendSpec` is exported and PTC-validated but has no `ElementMeta`, so `element_help` confidently reports no contracts and an invented kind) and **G1** (hard `MethodError` on a `Float32` beam with `Float64` params). **Verify before fixing**: this series' agent hit rate is ~60%, and part 7 §4 lists the four different ways their claims have come out. |
> | **The element metadata validator caught 1 of 13 injected defects** | **open (2026-08-04)** | [Part 7 §1](history/comprehensive_audit_2026_08_03_part7.md) — `validate_element_metadata` does not check that a declared contract is a contract, that a declared tracking method is one, that a declared default matches the constructor, or that a declared parameter is read; and two of its checks are **circular** (`supported_tracking_methods(T)` returns the very list being validated). `validate_configuration_metadata` is much stronger but enumerates hardcoded type literals, so a new solver/policy/observer is unchecked until someone edits the tuple — a fabricated `LyingSolver` with a wrong default passed it. This is the layer `AGENTS.md` says should be trusted *instead of* reading source. |
> | Audit the strong-strong solver stack | **open, narrowed again (2026-08-03)** | `interface.jl` (2,093) was read in full in [part 2](history/comprehensive_audit_2026_08_03_part2.md) and **`pic_cpu.jl` (1,715) in [part 3](history/comprehensive_audit_2026_08_03_part3.md)**. ~9,600 lines remain: `pic_cuda.jl` (5,807), `gaussian_pic_cuda.jl` (1,154), `spectral.jl` (1,045), `spectral_cuda.jl` (760), `gaussian_pic.jl` bulk. **The "contracts agree on the wrong answer" worry was correct, and part 3 caught it happening**: the default Green cache expands the mesh with the node count fixed, which changes the cell size and breaks the integer-cell alignment `green_type = :lattice` requires — both backends broke it identically, so CPU/CUDA parity passed at 1e-13 on the field of a source displaced 0.4 cells, and the only analytic field validation bypasses the cache. **`pic_cuda.jl` l. 1-2260 is now read in full** ([part 4](history/comprehensive_audit_2026_08_03_part4.md)) — **zero defects**: the specific question it was given, whether the CUDA path had inherited more of the CPU's defects the way it inherited S14, was answered no, with the duplicated device functions verified numerically equivalent and the `rho = 1` blind spot tested against an analytic reference. **`pic_cuda.jl` l. 1-3470 is now covered (60% of the file), still zero defects** — the field-solver extension is closed ([part 4 §7](history/comprehensive_audit_2026_08_03_part4.md)). It was settled by **measurement rather than reading**: the plane layout is CUDA-only (the CPU has no planes), the CPU side is independently verified, and CPU/CUDA parity measures **1e-15** against a **1e-5 to 1e-2** signal for any plane-level error — 10 to 13 orders of discriminating power, where the suite only asserts 1e-11. Two non-defect findings fixed (§8). **`pic_cuda.jl` l. 3470-5040 (device kernels) now covered too — ~87% of the file** ([part 5](history/comprehensive_audit_2026_08_03_part5.md)), which found **S18**: `CUDAPICLaunchConfig` is silently inert on a bare `collide!` (0 launch receipts vs 12 through a task), the same class as part 2's S1, now warning. Also closed a verification gap: the luminosity tree reduction's two power-of-two guards had **no test at all**, while a non-power-of-two block silently drops ~36% of the overlap grid. **Read next: `pic_cuda.jl` l. 5040-5810** — the device kernels (~1,340) and the Gaussian sequential path plus CUDA slicing (~770); the latter belongs with the still-unread `slicing.jl` (704). ~6,500 lines of the stack remain overall. |
