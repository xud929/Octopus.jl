...
> | ~~**Verify and fix the recorded findings from part 6**~~ | **closed except two perf items (2026-08-04)** | R1/R3/R4/R5/R6 were settled in [part 6 §8](history/comprehensive_audit_2026_08_03_part6.md); [part 9](history/comprehensive_audit_2026_08_04_part9.md) settled **R2** (ties-only, contract documented and pinned), **R7** (degenerate z now slice 1 on both backends and all methods), **R9** (dropped-charge tripwire — which immediately caught 83% of a slice silently discarded under strong kicks: the Dirichlet box is sized pre-collision, deposits happen after intra-collision kicks), **R10** (the `:grid_free` −1× mirror, unreachable pre-collision by box construction but reachable through the same kick growth; guarded), **R11** (`field_precision` now `inactive_backend` on CPU), the **Symbolics `[weakdeps]` extension** (both load modes verified; the suite round-trip is now unconditional), and the **method-overwrite guard** (a forced `Base.compilecache` in the suite — a runtime include is silent about overwrites on this Julia). **Still open: R8 and R12, both performance-only** — the CUDA `:equal_area` histogram (10–20×, and all work discarded at the default `nslices=1`) and the `n1·n2` spectral solves where `n1+n2` suffice. Each needs before/after benchmarks. |
...
> | ~~**Part 7 queue — 26 agent-confirmed findings**~~ | **closed (2026-08-04)** | [Part 8](history/comprehensive_audit_2026_08_04_part8.md) verified and fixed **T1, T3, T4, T5, K1, G1**; [part 9](history/comprehensive_audit_2026_08_04_part9.md) settled **everything else** — T2, T6–T11, G2–G4, C1–C3, K2–K8, each reproduced before its fix, with the corrections recorded beside the claims (G2's bit-parity fails even without neutralisation; G3's stale text was the constructor comment, not the main docstring; K2 resolved transitively by pinning the thin constructors bit-identical to the PTC-validated `ThinMultipoleSpec` runtime). The queue's own "one root cause" framing for the T-items was wrong (two mechanisms: nested `AbstractVector` for T3/T4, `LineEntry` for T1/T5), and G1 was **broader** than recorded: *any* non-Float64 rep threw. |
> | **The element metadata validator caught 1 of 13 injected defects** | **mostly closed (2026-08-04)** | [Part 9 §2](history/comprehensive_audit_2026_08_04_part9.md) replaced the circular checks: declarations are now validated against the architectural roots (a contract must be an `AbstractContract`, a method an `AbstractTrackingMethod`, an analysis an `AbstractAnalysis`), and every example must **compile to a declared runtime type** — the injected liar that validated clean now fails on three independent counts, pinned by a suite test that registers it temporarily. Still open, narrower: declared **defaults vs constructor** and declared-parameter-**is-read** checks, and `validate_configuration_metadata`'s hardcoded type enumeration (a new solver is unchecked until someone edits the tuple). |
