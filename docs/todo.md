...
> | Un-pin the remaining element runtimes from `Float64` | small, mechanical, but wants a test each | The lattice-magnet path (`LatticeMagnet`, `MisalignedElement`, `RefTilted`, `Patch`) now carries its parameters' number type, so a strength, length, curvature or misalignment can be a dual number, a TPS, complex or `BigFloat` — see `numeric_type`. About twelve other runtimes still declare `T<:AbstractFloat` and pin `Float64` at their entry point: `linear_maps.jl`, `crab_cavity.jl`, `linear6d.jl`, `lorentz_boost.jl`, `aperture.jl`, `thin_elements.jl`, `solenoid.jl`. Widening a bound is trivially safe; the entry point differs per element, so each wants a derivative test rather than a blind sweep. **Watch for the recurring bug:** any numeric literal converted through `T` and then *compared* must go through `real(T)` — `_curv_sin`, `_curv_vers`, `_atan_over` and the `abs(hL) < pi/2` guard each needed it. |
