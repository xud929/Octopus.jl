...
>
> **Start next: complete the lattice-magnet schemas (U9-2, row below).**
> The real repair its row prices: splice a shared `_LATTICE_BODY_PARAMS`
> into the six kinds the way `_PLACEMENT_PARAMS` is spliced, derived from
> `_LATTICE_BODY_KEYS` (already the authoritative list). The blast radius
> the row records — ~80 new entries into the effectiveness sweep, each
> verified or exempted with a reason — is exactly what the `alternatives`
> keystone and the central placement-DOF enrichment (closed 2026-08-15)
> just made tractable. After it: the U4-1 aperture probe redesign, whose
> shrinking-limit moves are now one `alternatives` declaration away once
> the probe survives.
>
