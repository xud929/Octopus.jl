...
> | **Part 7 queue — 26 agent-confirmed findings, NONE verified by me** | **head fixed (2026-08-04), 20 remain** | [Part 8](history/comprehensive_audit_2026_08_04_part8.md) verified and fixed **T1, T3, T4, T5, K1, G1** — all six survived verification, a first for this series, though the queue's own "one root cause" framing for the T-items was wrong (two mechanisms: nested `AbstractVector` for T3/T4, `LineEntry` for T1/T5 — the fix at the imagined single root would have closed only half) and G1 was **broader** than recorded: *any* non-Float64 rep threw, not just mixed precision, because the solver's own `kbb` is `Float64` by construction. `_bind_apertures` now asserts counts-vs-ids agreement at bind time, so a future walker divergence is a loud host error, not an unchecked device write. **Still open from part 7: T2, T6–T11, G2–G4, C1–C3, K2–K8** — the highest-value remainders are T6 (a failed `execute!` leaves duplicate turn labels and writes no loss file after a crash, the one artifact wanted then) and T2 (loss record `fits` omits the backend; hard `KernelError` reusing a task across CPU and CUDA reps). |
