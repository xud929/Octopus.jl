...
> | **2026-08-05_b audit open queue** | **open (2026-08-05)** | Second full re-read the same day, at `7de4d81`, because the prior pass declared `6a3f39ab` and HEAD was **63 commits / +9,125−775 lines** beyond it — that delta *is* the prior audit's fix campaign and queue-closure session, never itself audited. Four findings auditor-confirmed and **two fixed**: the strong-strong `.lum` prepare was destroying history a failed run never wrote (3 rows → 0; the prior U4-4 "fix" had only reversed the truncation window), and the curved solenoid's implicit-midpoint stage had no convergence check, returning residual **7.197 — or NaN — at its own default `nst`** for an ordinary detector solenoid. Two confirmed and left open with reproductions: the per-pair luminosity trace is populated on 1 of 6 CUDA PIC routes while the consuming contract passes by comparing an empty set (F1), and a `StrongStrongTask` has no loss accounting at all (A-1). **Start here to resume, in this order:** (1) the **15 unverified Major/High rows** — each is one reproduction away from a verdict, and two of them (U22-1, U24-1) refute numbers this repository has published; (2) the two confirmed-but-unfixed findings **F1** (a contract that passes by comparing an empty set) and **F7** (the PIC dropped-charge counter unreachable under `:node`/`:source_slice`); (3) the Medium tier. **Reproduce every lead before fixing it** — measured agent survival across this series is ~60%, and the miss shape that costs most is *right claim, wrong stated reason*, because the reason determines the fix. Do **not** start another reading pass: the declared scope is covered, and what is left is verification and repair. The worklist is the machine-extracted lead index at [`comprehensive_audit_2026_08_05_b_unit_reports/LEAD_INDEX.md`](history/comprehensive_audit_2026_08_05_b_unit_reports/LEAD_INDEX.md) lists **all 305 leads** with severity, location, claim and a status column (blank = not yet reproduced; 33 dispositioned so far). Pick a row there, then open its unit report in the same directory for the full mechanism and the **Repro line — the durable identity of a lead, since `file:line` rots**. The prose queue with the Major candidates called out is in [`comprehensive_audit_2026_08_05_b.md` §7](history/comprehensive_audit_2026_08_05_b.md) — read its header first: those are agent leads at ~60% historical survival, each worth one reproduction, not a fix. Highest first: the dropped-charge tripwire unreachable under `:node`/`:source_slice` (U6-1) and reporting **exactly zero in the worst regime it exists for** on CUDA spectral (U11-1); a **1.50× per-turn** performance regression from the thread-invariance fix (U6-2); two exception-replacement paths that hide the error that stopped a run (U13-2/3); CUDA `track!` drawing every turn at turn 0, variance **15.73×** (U14-1); `rng_id` namespace collision between beams and radiation (U14-2); kept-whole beam lines that cannot compile for CUDA (U15-1). |
> | The CPU hybrid moment pass computes cross-plane sums the CUDA twin gates | **open (2026-08-06, U10-3)** | `_gpic_source_moments` accumulates 14 sums unconditionally, including the four cross-plane ones (`sxy`, `sxpy`, `sypx`, `spxpy`) that are provably unreachable under the default `coupling_tol = Inf`: `_gpic_control_variate_mode` short-circuits on `isfinite(coupling_tol)` before it ever reads `bL.rxy`. The CUDA twin does gate this, selecting `Val(true)`/`Val(false)` and accumulating 14 vs 10 device slots, so the two backends disagree about whether the work is optional and the CPU pays always. Measured 33.4% overhead on the moment loop (0.0058 vs 0.0043 s/call at n = 2e6), which is ~0.3% of the interaction at the production point — the point is the asymmetry, not the size. The switch is available before the pass (`isfinite(gsolver.coupling_tol)`), and `_gpic_coupled_moments` is already documented as tolerant of moment tuples built without the cross terms, so the shape is a `Val`-typed parameter and an early-return NamedTuple without those fields. What stopped it being folded into a batch: `_gpic_boundary` calls `_gpic_drifted_covariance(mom, s)` unconditionally to build `a`, `b`, `d`, `rxy`, `lam`, `sigc`, so the tolerance has to be threaded through three more functions on the hybrid's hot path, and whether `lam`/`sigc` are read outside the coupled branch needs checking first. |
> | `_collision_solver` re-compares solver configurations every turn | **open (2026-08-06, U5-8)** | The identity-to-configuration change made `_collision_solver` build and compare two full `solver_configuration` NamedTuples once per collision per TURN, although solvers are immutable and `_preflight_solver_configurations!` already walks every collision once per `execute!`. The answer cannot change between turns. Measured: distinct-but-equal solvers cost 58,704 bytes and 78.3 us per call against 400 B / 0.36 us for a shared object; end to end that is a size-independent ~200 us/turn (grid 16 1.796 -> 1.994 ms/turn, 1.11x; grid 32 5.123 -> 5.332, 1.04x). Negligible at the production point (~0.3 s/turn) which is why it is not urgent, but it is pure waste on small problems. The fix is to resolve the solver once per `execute!` and cache it per collision label in `task.runtime_cache` — the same cache the U5-9 counter now uses — rather than per turn; deferred here only because it moves work across the preflight/turn-loop boundary and deserves its own gate. |
> | Two `BeamMomentObserver`s sharing one path silently interleave | **open (2026-08-06, U7-10)** | `_initialize_moment_file!` opens `"w"` unconditionally on a fresh object, truncating whatever another observer wrote, and `_discard_replayed_binary_rows!` then re-reads the count from disk so the surviving observer resyncs onto the OTHER writer's rows and appends after them. The file ends self-consistent and wrong, with no warning: measured 4 turns (size 1334, count 4), a second observer writing 2 turns (size 790, count 2), the first flushing again (size 1334) and the turn column reading back `[0.0, 1.0, 4.0, 5.0]` — turns 2 and 3 gone while the file reports 4 valid rows. Nothing documents path sharing for the BINARY observer, which is why this is filed as out-of-contract rather than a defect, but the `MomentObserver` docstring does document "a second task sharing the path", so a user may reasonably expect the same of its siblings. The fix is a process-wide registry of open observer paths, warning on the second writer — deferred because it needs a decision about whether the registry is per-process or per-task, and about whether two observers on one path should be an error rather than a warning. |
> | `cuda_track_lumped_rad_kernel!` uses CUDA's RNG, not the counter RNG | **open (2026-08-06, U14-7)** | The only GPU radiation path that does not use the counter RNG: it draws with `Random.randn(CUDA.default_rng(), T, N)`, so its results depend on CUDA's own RNG state rather than `set_global_rng!`, cannot match the CPU, and are not thread/layout invariant the way the counter RNG guarantees. It is unreachable from the policy API — `_requires_cuda_elementwise(elem)` without a ctx is `true`, but `_track_cuda_policy_elementwise!` consults the ctx-taking form, which is `false`, so inside any policy `track!` the element is fused and this kernel is skipped — and it stays reachable through the deprecated single-element `track!(rep, elem, turns, CUDABackend)`. No test covers it, and it allocates six N-length CuArrays per turn. Measured: `var(x) = 0.032156` at 16 turns, ratio to `turns*exc^2` = 1.0059, i.e. statistically right from a different and unreproducible stream. The decision it needs is whether to route it through the counter RNG like every other path or delete it with the deprecated entry point; either is a behaviour change to a public if deprecated API, which is why it is not folded into a batch. |
> | Slice boundaries and centers are not bit-identical CPU vs CUDA | **open (2026-08-06, U6-7)** | Slice MEMBERSHIP is identical on both backends in every case the audit could construct, but the boundaries and centers are not: `:normal_quantile` / `:specified` boundaries differ by up to 1363 ulps and `:equal_count` centers by up to 48,247 ulps. `_live_z_stats` accumulates mu and sigma serially while `_cuda_live_z_stats` uses device reductions, so the quantile boundaries `mu + sigma*q` differ in the last bits; and `_finish_longitudinal_slices` computes the centroid as `sum(i -> z[i], idx)` where `_cuda_slices_from_indices` uses `sum(@view z_host[hidx])` — different pairwise-reduction shapes over the same host data. Boundaries and centers feed the drift distances `sL/sR = (center - boundary)/2` directly, so this is a real backend divergence upstream of every kick, ~1e-16 absolute. It is upstream of the membership comparison that currently passes, which is why no existing check sees it. Fixing it means giving the two backends the same reduction shape for these two quantities — a parity-contract job spanning slicing.jl and its CUDA twin. |
> | The CUDA PIC per-pair luminosity sink is empty on three sub-routes | **open (2026-08-06, surfaced by U1-1, confirmed while disposing U4-9)** | `StrongStrongPICBackendConsistencyContract` FAILS at HEAD for `batch_mode=:sequential`: the CPU pushes one per-pair luminosity record per slice pair from the single loop that serves every configuration, and the GPU sequential route populates nothing, so `pair_keys_match` is false and the relative error is `Inf`. Measured across configurations: defaults, `green_cache=:none` and `(:slice_pair, :wavefront)` all pass with 18 records compared; `batch_mode=:sequential` fails with 0. U1-1 deliberately made this loud — its comment says "a non-empty CPU trace with an empty GPU one is a real backend divergence and is reported as such rather than silently waived" — and measured the same gap on `indexed=false`, `wavefront_fft=false` and `async=false` (0 records each, against 16 on every CPU route). What U1-1 did NOT do is record that the divergence itself is unrepaired, which is this row. The decision it needs: either populate `_ACTIVE_PIC_PAIR_LUMINOSITY_SINK` from the sequential and non-indexed CUDA kick routes so the trace exists on both backends, or declare per-pair tracing a wavefront-indexed-only feature and have the contract skip it by ROUTE rather than by what the CPU happened to produce. The first is more work and makes the contract stronger everywhere; the second is honest but narrows the check. Nothing in the suite runs `batch_mode=:sequential` through this contract, which is why a red public configuration went unnoticed. |
...
> | CPU and CUDA choose different working precision for slice moments | **open (2026-08-06, U3-4)** | For a Float32 beam the two twins disagree by 5.3e-7 relative on the kick, against 1.1e-13 for a Float64 beam — five orders of magnitude, purely from a type choice. CPU `_slice_transverse_moments` takes `T = promote_type(eltype(x), typeof(min_sigma))`, and `GaussianPoissonSolver()` defaults to `{Float64}`, so `min_sigma::Float64` silently promotes the whole moment computation and hence the kick to double precision even for a single-precision beam; the CUDA `_cuda_gaussian_moments_from_sums` takes `T = eltype(sums) = eltype(rep.x)` and never sees `min_sigma`. Measured: CPU `StrongTransverseMoments{Float64}` with sx = 1.0000351675065032e-4 against GPU `{Float32}` with 1.0000351e-4. Whichever convention is right, the two must pick the SAME one, and nothing enforces or documents it — constructing `GaussianPoissonSolver{Float32}` would align them. The decision is which convention: promoting to the solver's type (accuracy, and what the CPU does today) or following the beam (memory and speed, and what the GPU does). U3-7 is the same question for `_cuda_gaussian_collide_sequential!`'s return type. |
> | The CUDA PIC routes have no dropped-charge counter at all | **open (2026-08-06, U1-5)** | `grep -c dropped src/tasks/strongstrong/pic_cuda.jl` finds nothing but a comment. The CPU twin counts escapees on every interaction path, including the node path as of U6-1, and warns through `_pic_report_dropped`; the CUDA node routes deposit and interpolate with the zero-weight guard of `_cuda_pic_cic_weights` / `_cuda_pic_tsc_weights` and discard any escapee silently. Because BOTH twins used to share the gap on the node path, the CPU/CUDA parity contract could not see it — and now that only CUDA has it, parity still cannot see it, because a dropped particle changes both backends' fields identically. Measured consequence is unquantified precisely because the counter does not exist to read. The fix is a device-side counter (an `Int32` CuArray incremented atomically in the zero-weight branch, read back once per collide and routed through the same `_pic_report_dropped` warning), which is why it is not folded into a batch: it touches the hot deposition kernels. The CPU half of this lead is closed — U6-1 implemented the counting whose absence the `_pic_prebuild_node_caches!` docstring had been asserting was present. |
...
> | No coverage tripwire ties `_FOLDED_NAMED_STRENGTHS` to the fold sites | **open (2026-08-06, U15-7)** | The table in `beam_line.jl` that drives `_reject_folded_override` is hand-maintained against the call sites of `_fold_named_strengths`, and it has now been wrong **twice**: the thin kinds (U11-3) and then `:solenoid` one commit later (U15-7, fixed here, plus a `_FOLDED_TUPLE_KEYS` entry because a solenoid folds skew into `:kskew` and the default advice would have told the user to overwrite `ks`). Each miss makes a per-occurrence override written-reported-never-read. A derived tripwire is the right guard, and the obvious probes do **not** work: the kinds' example specs do not all store `:kn`/`:knl` (sbend and solenoid do not), and a generic "construct with a named strength and see if it is swallowed" sweep over the registry silently found *nothing*, which would be a check that cannot fail — the U20-1 shape. Deliberately not shipped for that reason; the solenoid case is pinned directly instead. Closing this needs the fold sites to declare themselves (e.g. `_fold_named_strengths` registering its kind), not a cleverer probe. |
> | Spectral transverse luminosity fold is worker-count-chunked | **open (2026-08-06, U18-2)** | The U5-1/U5-2 campaign moved the PIC deposit and the moment reduction to fixed chunk counts, but `spectral.jl`'s pair-batch luminosity reduction still sizes its chunk grid from `_cpu_worker_count()` (~line 1103, with the workspace pool sized to match at ~1000). So `spectral_t` luminosity moves with the thread setting: measured at n=15000 across 1/2/4 workers, max\|ΔL\| = 8.0 at 3 slices and 16.0 at 15 slices, i.e. 1.7e-16 and 2.7e-16 relative — about one ulp. Coordinates are bit-identical, and `pic`/`gpic`/`spectral_l` are exact. The suite now pins this as a documented envelope (coordinates exact, luminosity within 8 ulp) rather than omitting the arm, which is what it did before. Closing it exactly means giving the pair-batch reduction a fixed chunk grid, which changes the spectral workspace pool's sizing on low-thread runs — priced but unmeasured. todo.md's own earlier entry lists a documented cross-count envelope as an accepted resolution for this class, so this is a choice, not an oversight. |
...
> | ~~Nightly full-suite gate on GPU machines~~ | **installed (2026-08-05)** | CI has no GPU, so the CUDA half of the suite only runs where someone runs it — the dominant recorded failure class, and where the U15-4 device-IR regression hid. `test/nightly_suite.sh` runs the exact CI gate nightly and appends a date/commit/testsets/verdict/exit row to `~/.octopus_nightly/status.tsv`. **Opt-in per machine** (a manual crontab entry; installing the package never installs the gate); currently on the RTX 4500 Ada box only, first row PASS at 151 testsets. Details: `docs/current_runtime.md`. |
>
