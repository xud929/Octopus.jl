...
> | RF cavity velocity-slip term | **open (2026-08-05 audit, F16)** | `_rf_kick` converts with arc position `s = 0` because a runtime element has no channel to its accumulated reference path, so a ring closed by `ThinRFCavitySpec` has slip factor `alpha_c`, missing `-1/gamma0^2` — measured 1.84x nu_s error at 2.5 GeV proton / alpha_c = 0.2, wrong transition side when `alpha_c < 1/gamma0^2`; negligible at EIC-class gamma0, which is why every committed check passed. The theory note's own §7 criterion (nu_s against the **full** eta) is the missing check. Fix needs the arc/survey channel — the same one Scope B needs for `P0(s)`; fold them. Model boundary now documented on the element and in the note (correction block, §6 Step 0). |
