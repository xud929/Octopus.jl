...
>
> **Start next: the U4-1 aperture probe redesign (row below).**
> The `alternatives` mechanism (2026-08-15) already serves the
> shrinking-limit moves its row asks for; what remains is the survivable
> probe — a baseline where the particle LIVES, so an alive/dead flip is
> the observable a perturbation can produce — plus NaN-aware comparison
> per the row's two-things-together analysis. U9-2 closed 2026-08-15:
> all 23 body keys declared on all six lattice-magnet kinds from one
> merged table, all 83 new sweep entries checked (414 → 499), zero new
> exemptions, two retired.
>
...
> the remaining open rows after the one above are blocked-or-heavy
