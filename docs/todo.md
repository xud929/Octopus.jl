...
> | Legacy moment observers host-copy the whole beam per turn | **open (2026-08-11, neighbour audit N1)** | `BeamMomentObserver`/`JLD2BeamMomentObserver` route through `beam_statistics` → `_host_coordinate_arrays`: 48 MB D2H + host covariance per observed turn at 1M particles — the class U7-5 fixed in the BPM and the 2026-08-11 campaign fixed in `MomentObserver`. Low exposure (in no example or harness; `MomentObserver` is the documented choice). Priced: means + 6×6 cov + diagonal fourths = 33 fused-kernel power rows (two chunks), so a device path reuses the campaign machinery nearly verbatim — or document the pair as legacy. [`neighbour_audit_2026_08_11.md`](history/neighbour_audit_2026_08_11.md). |
