...
> | Three more truncating observer writers outside the U7-10 registry | **open (2026-08-07 neighbour audit, N3)** | `LuminosityObserver` (first `observe!` opens `"w"` off a per-object `initialized` latch), `BPMObserver` (same latch, plus `_bpm_discard_window!` rewriting the whole file from this object's memory — destroying any interleaved rows), and `CoordinateSnapshotObserver` under `append=false` (plus its replay truncation at a byte offset from this object's `written` map) all have the exact U7-10 silent-interleave shape and none call `_register_observer_path!`. Fix shape identical to U7-10: register at each truncating/first-write site, warn on a second live writer. Also recorded here: the task-level `.lum` path and the loss log (`write_loss_record` rewrites whole-file, idempotent per task, last-writer-wins ACROSS tasks) are outside any registry, so an observer and a task (or two tasks) sharing a path collide with no signal — that half needs a small design pass on what object identity a task-level writer registers. Details: [`docs/history/neighbour_audit_2026_08_07.md`](history/neighbour_audit_2026_08_07.md). |
> | Weak-strong strong-beam luminosity is not thread-count invariant | **open (2026-08-07 neighbour audit, N4)** | `_track_thin_strong_beam!` and `_track_gaussian_strong_beam!` fold per-particle luminosity through `zeros(T, policy.threads)` with STRIDED membership (`worker:nworkers:n`) — both the grouping and the membership of every partial change with the thread setting, so `elem.last_luminosity` moves with it: the exact class U5-1/U18-2 removed from the strong-strong stack, still live on the weak-strong path. No pin covers it (the thread-invariance testset sweeps strong-strong `collide!` only; the effectiveness contract's worker sweep compares coordinates only), and the CUDA twin folds in particle order — a third shape. Fix shape: a fixed-lane or particle-indexed fold (U6-7's recipe; the strided partition means U18-2's slice-indexing does not map directly). |
> | `_masked_rms` has per-backend reduction shapes upstream of the spectral box | **open (2026-08-07 neighbour audit, N5)** | CPU is a strict serial loop, CUDA a tree reduction, and the CPU's own masked/unmasked paths differ from each other — feeding `L = max(d*smax, 1.05*emax)`, the Dirichlet box every spectral kick is solved on, where `d*smax` dominates in the documented production regime. The U6-7 class; the 2026-08-05_b audit's U11 report classified it "sequential vs tree, equivalent" without a row. Fix shape established: the canonical lane fold. Same divergence in the 6D twin. |
> | `Val{COMPUTE_LUMINOSITY}` gates the soft-Gaussian kick bodies | **open (2026-08-07 neighbour audit, N6)** | CPU `_apply_slice_kick_one!` and the CUDA sequential `_cuda_slice_kick_kernel!` compile two specializations of the whole kick body to gate only the returned density, and BOTH are reachable for the same beam by flipping `gaussian_when_luminosity` — the U10-3 second-specialization FMA-contraction mechanism, latent only because nothing pins coordinates bit-equal across the flip. The CUDA fused route already gates at runtime (`seg_complum::Int32`), so three routes carry two gating disciplines; converting the other two to runtime gating (U10-3's fix shape) unifies them. Sibling noted: `_slice_transverse_moments`' 14-vs-10 `Val{COUPLED}` loop is the surviving twin of the shape U10-3 converted away in gpic — mitigated by `COUPLED` being a per-solver type parameter, so flag only if coupled-reduces-to-uncoupled is ever pinned bit-exactly. |
> | Float32 PIC luminosity types and envelope are unpinned | **open (2026-08-07 neighbour audit, N7)** | For a Float32 beam the PIC CPU `collide!` returns Float64 (promoted through `kbb`) while CUDA returns Float32 (beam-typed lum buffers) — the U3-7 return-type asymmetry in the solver the Gaussian fix did not touch. Measured: Float32 PIC cross-backend kick parity 1.55e-6 relative (3.2e-15 at Float64), plausibly the honest single-precision envelope since PIC fields are beam-typed on BOTH backends — a symmetric convention, unlike the Gaussian case — but nothing pins either the types or the envelope. The decision is the U3-4 one again: PIC's natural convention may genuinely be follow-the-beam (both backends' field math already is), in which case the CPU return should demote rather than the CUDA promote — decide, align, document, pin. |
> | Dead global-RNG code in `Beam.jl` | **open (2026-08-07 neighbour audit, N8, minor)** | `_default_rng` (both backend methods) has zero call sites repo-wide, and `_alloc_randn`'s `rng === nothing` fallbacks to `Random.default_rng()`/`CUDA.default_rng()` are unreachable from the one guarded caller — live code outside the counter-RNG guarantee that a future caller would silently pick up. Delete the dead functions and make the fallbacks an explicit error, or route them through the counter. |
