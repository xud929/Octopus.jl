...
> | **Beam line / sequence** | **designed, ready to implement** | Design settled against MAD-X, Bmad, elegant, AT and JuAcc: [`docs/theory/beam_line_composition.md`](theory/beam_line_composition.md). Flat expansion with per-placement provenance, assemblies as tag sets, one `sel"ARC1/CQS[3]"` selector, a line is an `ElementSpec` so girder-style assembly misalignment is free. Subsumes **element names fleet-wide** — the line is the second consumer that item was waiting for. **Start here.** |
