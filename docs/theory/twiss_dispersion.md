...
3. **Momentum dispersion:** compare with a finite difference of the coasting closed orbit using the same momentum and coordinate convention. (Discharged 2026-09-15 against MAD-X twiss at deltap=+-0.0001 and against xtrack 0.112.0, `validation/twiss_madx_benchmark.jl` and `validation/twiss_xsuite_benchmark.jl`; the MAD-X finite-difference witness agrees to 3.8471258401173714e-09 and is recorded, the converted dispersion is gated at 1e-12 relative.)
...
7. **Ohmi and Xsuite conventions:** verify (O2)–(O5) and (X2) after coordinate conversion; distinguish full-6D normalization from forced 4D responses. (Discharged 2026-09-15: (X2) after the conversion Sh(-C/gamma0^2) against xtrack 0.112.0, the full-6D normalization against PTC ptc_twiss time=false after F = diag(1,1,1,1,-1,1), and the forced-4D versus full-6D distinction by the C3 T6 cell, gated; `validation/twiss_xsuite_benchmark.jl`, `validation/twiss_ptc_benchmark.jl`.)
...
Status 2026-09-15: items 3 and 7 are discharged by the stage 8 external benchmarks recorded in `docs/history/twiss_external_benchmarks_2026_09_15.md`; items 1, 2, 5 and 9 are exercised there in part (Courant–Snyder functions, both signs of $\det R$, the 6D cells, the degenerate fixtures) but keep their status as requirements; items 4, 6 and 8 are untouched.
