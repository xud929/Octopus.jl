...
solver = PICPoissonSolver(; slicing=slicing, grid=(config.grid, config.grid),
                          deposit_method=config.deposit, green_type=:integrated,
                          green_cache=:slice_pair, longitudinal_kick=true,
                          slice_interpolation=config.scheme,
                          interaction_grid=config.gridmode,
                          field_derivative=Symbol(get(ENV, "OCTOPUS_EMIT_DERIV", "second")))
