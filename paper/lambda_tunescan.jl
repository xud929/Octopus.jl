...
function measure(; solver_kind, seed, turns, n_macro, xi=0.005, aspect=1.0, qx=0.31)
...
# Yokoya factor versus WORKING POINT at fixed flat aspect ratio (r=0.09).
# The tune-swap control in the manuscript measures dLambda/dQ ~ +0.6 over a
# 0.01 tune step; this scans a wide range to see whether the wide-plane
# plateau (1.25-1.27 at Q=0.31) is set by proximity to the third integer.
for qx in (0.18, 0.24, 0.28, 0.31, 0.36, 0.42)
    @time (Lx, Ly) = measure(solver_kind=:pic, seed=1, turns=4096,
                             n_macro=20000, aspect=0.09, qx=qx)
    println("tunescan qx=", qx, "  Lambda_x=", round(Lx; digits=4))
    flush(stdout)
end
