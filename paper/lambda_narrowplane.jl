...
for r in (1.0, 0.55, 0.30, 0.16, 0.09, 0.05)
    println("aspect ", r)
    @time (Lx, Ly) = measure(solver_kind=:pic, seed=1, turns=2048, n_macro=20000, aspect=r)
    # Ly returned as (qpi-qsig)/xi_x; narrow-plane uses xi_y = xi_x / r
    println("narrowplane r=", r, "  Lx=", round(Lx; digits=4), "  Ly_over_xix=", round(Ly; digits=4),
            "  Ly=", round(Ly * r; digits=4))
end
